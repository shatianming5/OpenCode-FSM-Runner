...
from .aider_client import create_coder
...
def run(config: RunnerConfig) -> int:
    ...
    fnames = [config.plan_rel]
    if config.pipeline_rel:
        fnames.append(config.pipeline_rel)
    coder = create_coder(model_name=config.model, fnames=fnames)
    ...
    for iter_idx in range(1, config.max_iters + 1):
        ...
        if next_step is None:
            ...
            _coder_run(
                coder,
                f"/test {config.effective_test_cmd}",
                log_path=log_path,
                iter_idx=iter_idx,
                fsm_state=state["fsm_state"],
                event="aider_test",
